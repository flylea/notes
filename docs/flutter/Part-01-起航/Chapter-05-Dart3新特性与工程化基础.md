...
> **上一章**: [Chapter 04 — Dart 核心语法速通（下）：面向对象与集合](./Chapter-04-Dart核心语法速通-下.md)
...
> 如果你用过 TypeScript，这类似于 discriminated union + switch 的穷尽性检查。在 Dart 中，sealed class + switch 编译器会保证你处理了所有可能的子类型，少一个都会编译报错——这比传统 `if (x is Success)` 安全得多。
...
> **"健全"（sound）是什么意思？** "健全"是类型系统的术语，意思是：编译器承诺的事情，运行时一定会遵守。如果编译器说 `String name` 不可能是 null，那么在 App 运行的每时每刻、任何代码路径上，`name` 都绝不可能是 null。这种可靠性是编译器在编译时通过静态分析保证的，没有运行时漏洞。对比 TypeScript 的类型系统——它是"非健全"的（你可以用 `as any` 绕过），所以运行时仍然可能出现类型错误。Dart 的空安全是真正可靠的。
...
1. 用 sealed class 定义 `ApiResult<T>` 的三个子类（Success/Failure/Loading），写一个函数对 `ApiResult<Book>` 做 switch 模式匹配，分别打印不同内容
2. 给 Library App 项目的 `analysis_options.yaml` 添加 `strict-casts: true` 和 `strict-raw-types: true`，运行 `dart analyze` 并修复所有警告
3. 用 record 写一个函数返回 `(String title, int count)`，在调用处用模式解构分别取出值

验证标准：`dart analyze` 零问题，switch 表达式覆盖所有 sealed 子类。
