...
| 常见错误 | 后果 | 正确做法 |
|---------|------|---------|
| 只测 `fromJson` 正常 JSON，不测缺失字段 | 字段缺失时解析崩溃或数据错误，线上白屏 | 覆盖 minimal JSON（仅必填字段）、null 字段、空字符串等边界 |
| 用 `==` 比较两个 `Book` 实例而非 `equals()` | freezed 值对象可能 `==` 比较引用而非值，测试误通过 | 使用 `expect(a, equals(b))` 而非 `expect(a == b, ...)` |
| 不验证 `copyWith` 的不可变性（原对象未变） | `copyWith` 可能意外修改原对象，引发状态管理的 bug | 测试中显式断言原对象的字段未被修改 |
| `toJson` / `fromJson` 往返测试只比部分字段 | 遗漏字段导致数据丢失，难以排查 | 遍历所有字段逐一比对，确保完整往返 |
| 测试数据硬编码但不同步更新 | 模型字段增删后测试数据过期 | 用工厂函数 `createTestBook()` 集中管理测试数据 |

**最佳实践**：

- 使用 `group()` 按方法分区（`fromJson` / `toJson` / `copyWith` / `getters`）
- 为每个模型枚举的合法值和不合法值各写一条测试
- `toJson`→`fromJson` 往返测试是必测项，验证序列化和反序列化的对称性
- freezed 模型测试应全部为纯函数测试，执行速度极快（<1ms 每条）
- 先写测试数据（Arrange），再执行（Act），最后验证（Assert）的 AAA 模式
- 模型测试的覆盖率目标为 95%+，因为它们逻辑最简单、影响面最大
- 使用 `flutter_test` 而非纯 `test` 包以获得更好的 Flutter 类型支持
