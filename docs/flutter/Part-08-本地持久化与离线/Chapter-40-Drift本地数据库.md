...
> **上一章**: [Chapter 39 — SharedPreferences](./Chapter-39-SharedPreferences用户偏好.md) | **下一章**: [Chapter 41 — 离线优先策略](./Chapter-41-离线优先策略.md)
> **官方文档**: [pub.dev/packages/drift](https://pub.dev/packages/drift) | [drift.simonbinder.eu](https://drift.simonbinder.eu)

---

# 第 40 章：Drift — 本地关系型数据库
...
> **注意 `text()()` 这种"双层括号"写法。** 这是 Dart 的 Builder 模式——`text()` 创建了一个文本列的"配置器"对象，后面的 `()` 表示"配置完了，用默认设置生成最终的列定义"。`nullable()()` 同理：先声明"允许为空"，再结束构建。
...
> **@DriftAccessor** 是 Drift 的注解，告诉 build_runner 工具"这个类需要生成对应的辅助代码"。运行 `dart run build_runner build` 后，`_$BookDaoMixin`、`.g.dart` 文件等就会自动生成——包括增删改查的快捷方法。这是 Dart 代码生成（Code Generation）的标准模式。
>
> **Companion 与 Value**：Drift 用 Companion 类表示"要插入或更新的数据"。注意字段被包装在 `Value<T>` 中——不是直接写 `status: 'returned'`，而是写 `status: Value('returned')`。原因是：更新时如果你只想改 status 不改其他字段，可以用 `Value.absent()` 表示"这个字段别动"。
...
class AppDatabase extends _$AppDatabase {
  ...
  @override MigrationStrategy get migration => MigrationStrategy(
    beforeOpen: (details) async {
      ...
      // WAL（Write-Ahead Logging）模式让 SQLite 支持"边读边写"——
      // 类比：传统模式下一个人在黑板上写字时要锁门不让别人看，写完才开门；WAL 模式下在草稿纸上写，写完再贴到黑板上，别人可以同时看黑板。
      await customStatement('PRAGMA foreign_keys=ON');
      // PRAGMA foreign_keys=ON 是开启外键约束——SQLite 默认关闭，关闭意味着删了一条图书后，借阅记录中的 bookId 可以指向不存在的书。
    },
...
1. **新增 ReviewCache 表**：在 `tables.dart` 中定义 `ReviewCache` 表，包含字段：`id`（主键）、`bookId`、`userId`、`rating`（实数，0-5）、`content`（文本，可为空）、`createdAt`（日期时间）。运行 `dart run build_runner build` 生成代码，创建 `ReviewCacheDao` 包含 `insertReview`、`getReviewsByBook`、`deleteReview` 三个方法。验证标准：表在数据库中正确创建；插入一条评论后通过 `getReviewsByBook` 能查到。

2. **实现缓存图书的分页查询**：在 `BookDao` 中添加 `watchBooksPaged(int offset, int limit)` 方法，使用 Drift 的 `limit()` 和 `offset()` 子句，返回 `Stream<List<BookCacheData>>`。配合 Riverpod 的 `family` Provider 实现图书列表页的无限滚动加载——每次滚动到底部时增加 offset 并追加数据。验证标准：首页加载前 20 本书；滚动到底部自动加载后续 20 本；数据不会重复。

3. **编写数据库迁移测试**：使用 `NativeDatabase.memory()` 创建内存测试数据库，在 `test` 目录下编写测试：先创建 schemaVersion 1 的数据库并插入测试数据，然后打开 schemaVersion 2 的数据库（触发 `onUpgrade` 迁移），验证旧数据完整保留且新字段有正确默认值。验证标准：运行 `flutter test` 通过；迁移后旧数据 row 数不变；新表的默认值符合预期。

验证标准：所有练习编译通过，数据库测试用 `flutter test` 验证，分页功能在模拟器中手动验证。

---

> **下一步**: [Chapter 41 — 离线优先策略](./Chapter-41-离线优先策略.md)
