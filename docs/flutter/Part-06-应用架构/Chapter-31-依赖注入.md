> **Part**: Part VI — 应用架构
> **上一章**: [Chapter 30 — Repository 模式](./Chapter-30-Repository模式.md)
> **下一章**: [Chapter 32 — Feature-First 项目结构](./Chapter-32-FeatureFirst项目结构.md)
> **官方文档**: [flutter.cn/app-architecture/recommendations](https://docs.flutter.cn/app-architecture/recommendations)

---

# 第 31 章：依赖注入 — Riverpod DI 体系 + get_it 共存
...
2. 用 `get_it` 管理非响应式服务：注册 `AnalyticsService` 和 `LoggerService` 为 get_it 单例（`registerLazySingleton`），在 `main()` 中初始化后，ViewModel 中通过 `getIt<AnalyticsService>()` 调用。验证标准：两个服务全局唯一，不参与 UI 重建。

3. 实现 dev/staging/prod 三环境切换：创建 `AppConfig` 抽象类，包含 `baseUrl` / `apiKey` / `environment` 三个属性，分别实现 `DevConfig` / `StagingConfig` / `ProdConfig`。使用 `--dart-define=ENV=staging` 环境变量控制 Provider 注册哪个配置。验证标准：切换 `ENV` 值后 App 连接到对应环境的 API。

> **下一步**: [Chapter 32 — Feature-First 项目结构](./Chapter-32-FeatureFirst项目结构.md)
