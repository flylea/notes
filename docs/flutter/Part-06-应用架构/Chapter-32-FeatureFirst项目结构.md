> **Part**: Part VI — 应用架构
> **上一章**: [Chapter 31 — 依赖注入](./Chapter-31-依赖注入.md)
> **下一章**: [Chapter 33 — App 生命周期管理](./Chapter-33-App生命周期管理.md)
...
# 第 32 章：Feature-First 项目结构与代码规范
...
2. 为 `features/books/` 创建 barrel file：在 `lib/features/books/books.dart` 中导出所有公开 Widget（`BookCard` / `BookGridItem` / `BookListPage` / `BookDetailPage`）和所有公开 ViewModel，其他 feature 通过 `import 'package:library_app/features/books/books.dart'` 导入。验证标准：一个 import 语句即可使用 books feature 的全部公开 API。

3. 添加 Feature 边界 lint 规则：在 `analysis_options.yaml` 中配置 lint 规则，禁止 `features/books/` 内部文件直接 `import '../auth/screens/login_page.dart'`（跨 feature 的内部文件引用），强制通过 barrel file 或 `core/` 层通信。验证标准：`dart analyze` 能检测到跨 feature 的内部 import 并报错。

> **下一步**: [Chapter 33 — App 生命周期管理](./Chapter-33-App生命周期管理.md)
