...
| 常见错误 | 后果 | 正确做法 |
|---------|------|---------|
| AI API Key 直接写在客户端 Dart 代码中 | 任何人都可从 APK/IPA 反编译提取 Key，被盗刷 | 通过 Supabase Edge Function（或 Cloudflare Workers）代理 API 调用 |
| AI 生成的 Widget 代码不经 `flutter analyze` 和测试直接提交 | 引入语法错误、布局溢出或运行时异常 | AI 输出必须通过 lint + test 门禁 |
| GenUI 产出的代码直接上生产 | 实验阶段 API 不稳定，代码质量和样式不可控 | GenUI 仅用于原型和灵感探索，生产代码需人工审查优化 |
| MCP Server 开放写文件权限给不信任的 AI 模型 | AI 可能意外删除文件或覆盖关键代码 | 在 Git 仓库中操作，利用版本控制回滚 AI 的错误修改 |

**最佳实践**：

- AI API Key 安全三原则：永远走 Edge Function 代理、Key 存环境变量、开访问频率限制
- 使用 AI 生成代码后运行 `flutter analyze` + `flutter test` 作为质量门禁
- MCP（Model Context Protocol）让 AI 直接与 Flutter 工具链交互，实现 Agentic Hot Reload
- A2UI（Agent-to-UI）协议是未来方向，但当前阶段仍应关注代码生成的质量
- AI 书评/推荐功能应标注"AI 生成"以保持透明度
- 在本地测试 AI 功能时使用测试 API Key，上线前切换为生产密钥
- 定期审查 AI 工具更新的 SDK 版本，跟进 GenUI / AI Toolkit 的正式发布
