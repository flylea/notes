...
| 常见错误 | 后果 | 正确做法 |
|---------|------|---------|
| Keystore `.jks` 文件没有备份且丢失 | 永远无法更新已发布的 Android App，只能换包名重新发布 | 生成后立即备份到密码管理器或加密存储，加入 `.gitignore` |
| iOS 证书过期未续签 | CI 构建失败，TestFlight 无法上传新版本 | 定期检查证书有效期，设置日历提醒（证书有效期 1 年） |
| 发布前忘记更新 `pubspec.yaml` 版本号 | Play Store / App Store 拒绝上传（版本号重复） | CI 中加入版本号检查步骤，或者在 fastlane 中自动递增 |
| `Info.plist` 隐私描述使用英文或过于笼统 | Apple 审核被拒（4.8 条款） | 隐私描述使用中文清晰说明用途："用于扫描ISBN条码录入图书" |
| 应用图标和闪屏留到上线前才配置 | 多平台多尺寸图标生成和调试需要大量时间 | 在 UI 成形后（Part-02 结束后）就完成图标和闪屏配置 |

**最佳实践**：

- Android Keystore 信息（密码/别名/Base64）通过 CI Secrets 管理，不写入代码
- iOS 签名使用 Fastlane Match 管理证书和 Profile，告别手动导出 `.p12`
- 发布前运行完整检查清单：签名、版本号、图标、权限描述、Privacy Manifest
- Android 15+ App 默认 edge-to-edge，验证所有页面的 SafeArea 和底部导航不被遮挡
- iOS 必须配置 `PrivacyInfo.xcprivacy`（2024 起），声明所有隐私 API 用途
- Google Play 2024 新规：新个人开发者需 20+ 人封闭测试 14 天后才能申请正式发布
- 使用 `--dart-define` 实现多环境构建：dev/staging/prod 三套 Supabase 连接
...
> **下一步**: [Chapter 61 — CI/CD](./Chapter-61-CICD.md)
