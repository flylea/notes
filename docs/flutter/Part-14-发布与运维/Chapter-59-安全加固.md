...
| 常见错误 | 后果 | 正确做法 |
|---------|------|---------|
| 认为 `anonKey` 隐藏了就等于安全 | anonKey 内嵌在二进制中，任何人都可从 APK/IPA 提取，形同虚设 | 安全完全依赖 RLS（Row Level Security），不依赖 Key 隐藏 |
| 数据库表忘记 `ENABLE ROW LEVEL SECURITY` | 任何持有 anonKey 的人都可以对该表执行任意 SQL | 每张表上线前都执行 `ALTER TABLE xxx ENABLE ROW LEVEL SECURITY` |
| 混淆后丢弃 `--split-debug-info` 符号文件 | 线上崩溃堆栈永久无法还原 | 符号文件备份到安全位置并上传 Sentry |
| API Key 或 Token 存储在 `SharedPreferences` | 明文存储在设备上，root 后可轻易读取 | 始终用 `flutter_secure_storage`（系统 Keychain/KeyStore 加密） |
| 敏感数据加密但加密密钥硬编码在代码中 | 密钥随代码混淆但仍可被提取 | 使用系统提供的 Keychain/KeyStore 保管加密密钥 |

**最佳实践**：

- OWASP Mobile Top 10 作为安全检查清单，每次发版前逐条自查
- RLS Policy 使用 `USING` （读校验）+ `WITH CHECK`（写校验）双重保险
- 生产环境关闭详细日志：ProGuard 移除 `android.util.Log.d/i/v`，Dart 端 Logger Level WARNING+
- 证书固定（Certificate Pinning）对金融类 App 必须，普通 App HTTPS + RLS 已足够
- `flutter pub outdated` 每月运行一次，及时修复已知 CVE
- 越狱/root 设备检测采用"提示而非阻止"策略，避免误判影响正常用户
- 服务端 API Key 始终通过 Supabase Edge Function 代理，永远不嵌入客户端二进制
...
> **下一步**: [Chapter 60 — 多平台部署](./Chapter-60-多平台部署.md)
