...
> **上一章**: [Chapter 10 — 滚动与列表](./Chapter-10-滚动与列表.md)
> **下一章**: [Chapter 12 — Material 3 主题系统](./Chapter-12-Material3主题系统.md)
...
键盘弹出时遮挡输入框是移动端最常见的体验问题。

### 7.1 Scaffold 自动处理

```dart
Scaffold(
  resizeToAvoidBottomInset: true,  // 默认 true — 键盘弹出时 Scaffold 自动上移
  body: /* ... */,
);
```

这个属性默认开启。如果某些场景下不想让 Scaffold 上移（如聊天界面的背景图），设为 `false`。
...
```dart
// 方案 1：SingleChildScrollView + reverse（聊天/评论框常用）
Scaffold(
  body: SingleChildScrollView(
    reverse: true,  // 键盘弹出时自动聚焦到末尾
    child: Column(
      children: [
        // ... 上面的内容
        TextField(
...
**1. GlobalKey\<FormState\> 表单校验**

为图书馆 App 实现一个完整的图书添加表单，要求通过 `GlobalKey<FormState>` 驱动的 `validate()` 和 `save()` 方法完成提交流程。请实现：

- 必填字段：书名（至少 2 个字符）、作者（至少 2 个字符）、ISBN（不能为空，格式为纯数字或连字符）
- 可选字段：封面 URL（如果填写，必须以 `http://` 或 `https://` 开头）、简介（不超过 500 字）
- 使用 `TextFormField`（而非 `TextField`），每个字段提供独立的 `validator` 函数，返回 `null` 表示验证通过，返回 `String` 表示错误信息
- 设置 `autovalidateMode: AutovalidateMode.onUserInteraction`
- 提交按钮按下时调用 `_formKey.currentState!.validate()`，通过后调用 `save()` 收集所有字段值，在 console 打印收集到的图书数据
- 验证：新建 `lib/screens/book_add_screen.dart`，运行后分别测试以下场景：
  - 所有字段留空直接点提交 -> 各字段显示红色错误提示
  - 只填书名不填作者 -> 作者字段显示"作者为必填"
  - 填写正确数据点提交 -> console 打印完整图书数据，表单提交成功
  - ISBN 填入"abc" -> 显示"ISBN 格式不正确"

**2. TextEditingController 双向绑定与实时搜索**

为图书馆 App 的搜索栏实现实时过滤功能，要求使用 `TextEditingController` 驱动搜索框与结果列表的双向绑定。请实现：

- 搜索框使用 `TextField`，绑定一个 `TextEditingController`
- 在 `initState` 中通过 `addListener` 监听文本变化，实现 300ms 防抖（使用 `Timer`）
- 防抖触发后，从 `sampleBooks` 列表中过滤出书名或作者包含搜索关键词的图书（忽略大小写）
- 过滤后的结果列表用 `ListView.builder` 展示（每项显示书名和作者）
- 搜索框右侧有一个清除按钮（`suffixIcon`），点击后清空输入框并恢复显示全部图书
- 在 `dispose` 中正确释放 `TextEditingController`
- 验证：新建 `lib/screens/book_search_screen.dart`，运行后输入"clean" -> 过滤出"Clean Code"；清空搜索框 -> 恢复全部图书。打开 Flutter DevTools 的 Memory 面板，进出页面 5 次确认没有内存泄漏（controller 被正确 dispose）。

**3. 自定义 InputDecoration 美化搜索框**

为图书馆 App 设计一个美观的搜索框，要求使用自定义的 `InputDecoration` 样式。请实现：

- 使用 `TextField` + `InputDecoration` 配置以下样式：
  - `hintText`："搜索图书、作者或 ISBN..."
  - `prefixIcon`：搜索图标（`Icons.search`），颜色为主题色
  - `suffixIcon`：过滤图标按钮（`Icons.filter_list`），点击后在 console 打印"打开筛选面板"
  - `filled: true`，`fillColor` 使用 `Theme.of(context).colorScheme.surfaceContainerHighest`
  - `border`：`OutlineInputBorder`，圆角 12，无边框线（`BorderSide.none`）
  - `enabledBorder` 和 `focusedBorder`：圆角 12，聚焦时边框为主题色、宽度 2
  - `contentPadding`：水平 16、垂直 12
  - 搜索框总宽度填满父容器，高度自然由内容撑开
- 给搜索框外层添加 `Padding(horizontal: 16, vertical: 8)`
- 验证：在 `HomeScreen` 中替换现有的 `_buildSearchBar()` 实现，运行确认搜索框外观与 Material 3 设计规范一致，点击过滤图标时 console 正确输出。在 Android 和 iOS 模拟器中分别确认搜索框在两种平台上的默认渲染效果。

---

> **下一步**: [Chapter 12 — Material 3 主题系统](./Chapter-12-Material3主题系统.md)
