...
> **上一章**: [Chapter 09 — 布局系统精讲](./Chapter-09-布局系统精讲.md)
> **下一章**: [Chapter 11 — 表单与用户输入](./Chapter-11-表单与用户输入.md)
...
**1. ListView.builder 构建图书列表**

在图书馆 App 中，借阅记录页面需要展示一个历史借阅记录列表。每条记录包含：书名、借阅日期、归还日期、状态（"已还"/"借阅中"/"逾期"）。请实现：

- 使用 `ListView.builder` 构建列表（假设有 20 条记录，用 `List.generate` 模拟数据）
- 每个列表项使用 `ListTile` 或自定义 `Card` 布局
  - `leading`：图书封面缩略图（`CircleAvatar` 或小尺寸 `Image`）
  - `title`：书名（单行，溢出省略号）
  - `subtitle`：借阅日期 ~ 归还日期
  - `trailing`：状态标签（用 `Container` + 不同颜色区分三种状态）
- 列表顶部添加一个 `Padding` 包裹的汇总文字："共 20 条借阅记录"
- 验证：新建 `lib/screens/borrowing_history_screen.dart`，在 `HomeScreen` 的"借阅" Tab 中导航到此页面，滚动列表确认所有 20 条记录展示正常，状态颜色正确区分。使用 Flutter DevTools 的 Performance 面板确认滚动帧率保持 60fps（无明显 jank）。

**2. GridView 分类网格**

图书馆 App 的发现页需要一个按图书分类浏览的网格界面。每个分类卡片显示分类图标 + 分类名称 + 该分类下的图书数量。请实现：

- 使用 `GridView.builder` + `SliverGridDelegateWithFixedCrossAxisCount`（2 列）
- 数据源为 8 个分类：技术、科学、文学、历史、哲学、艺术、经济、其他
- 每个分类卡片用 `Card` + `Column` 布局：
  - 顶部：分类图标（`Icon`，大小 48，颜色为主题色）
  - 中间：分类名称（`Text`，字号 16，粗体）
  - 底部：图书数量（`Text`，字号 12，灰色，格式"12 本"）
- `childAspectRatio` 设为 1.0（正方形卡片），`crossAxisSpacing` 和 `mainAxisSpacing` 均为 12
- 验证：新建 `lib/screens/category_grid_screen.dart`，点击分类卡片在 console 打印分类名称。运行确认 8 个分类卡片以 2 列网格排列，滚动顺畅。尝试将 `crossAxisCount` 改为 3，确认列数变化后布局自动适应。

**3. SliverAppBar 可伸缩头部**

图书馆 App 需要一个作者详情页，顶部展示作者信息，支持向下滚动时头部折叠、向上滚动时头部展开。请实现：

- 使用 `CustomScrollView` + `SliverAppBar`（`pinned: true`, `expandedHeight: 250`）
- `FlexibleSpaceBar` 中：
  - `title`：作者姓名
  - `background`：用 `Stack` 叠加——底层放一张占位图（`Container` + 渐变色背景），上层在底部放作者姓名和生卒年份（白色文字）
- 头部下方使用 `SliverList.builder` 展示该作者的图书列表（至少 10 本，用 `List.generate` 模拟）
- 每本书用 `ListTile` 展示（leading 封面、title 书名、subtitle 出版年份）
- 验证：新建 `lib/screens/author_detail_screen.dart`，运行后向上滚动，观察头部逐渐折叠为导航栏标题，且标题栏始终固定在顶部（`pinned`）。向下滚动到顶部确认头部完全展开。将 `pinned` 改为 `false` 后再测试，对比两种行为的差异。

---

> **下一步**: [Chapter 11 — 表单与用户输入](./Chapter-11-表单与用户输入.md)
