...
> **上一章**: [Chapter 08 — Cupertino 组件](./Chapter-08-Cupertino组件.md)
> **下一章**: [Chapter 10 — 滚动与列表](./Chapter-10-滚动与列表.md)
...
// 四种常见约束类型：
// ① tight — min == max（固定尺寸）
//    场景：屏幕强制 Scaffold 填满全屏 → Scaffold 收到 tight(屏幕宽, 屏幕高)
...
//    场景：Container 给子 Widget 约束 "最大宽200高100但不强制" → 子 Widget 可以选更小的尺寸
BoxConstraints.loose(Size(200, 100));

// ③ unbounded — max=infinity（无限约束）
//    场景：ListView 在滚动方向给子 Widget 无限高度 → 子项可以任意高，由内容决定
const BoxConstraints();

// ④ expanding — min=0, max=infinity（填满可用空间）
//    场景：Scaffold 的 body 区域 → "填满 AppBar 以下的所有剩余空间"
...
class BookDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // Sliver 是 Flutter 的一种特殊布局系统——它可以按需渲染（只构建屏幕上可见的部分），
      // 适合长列表和可折叠的头部。Sliver 的完整讲解在 Ch10 滚动与列表。
...
在图书馆 App 的图书详情页中，需要展示一条信息栏，水平排列三项信息（ISBN / 分类 / 评分），三项等分可用宽度。请实现：

- 使用 `Row` + 三个 `Expanded(flex: 1)` 子项实现等分
- 每项包含一个图标（`Icons.tag` / `Icons.category` / `Icons.star`）和一行文字
- 每项内部用 `Column` 垂直排列图标和文字，居中对齐
- 给每项添加 `Container` 包裹，背景色为 `Colors.grey[100]`，圆角 8，内边距 12
- 验证：新建 `lib/widgets/book_info_bar.dart`，在 `BookDetailScreen` 中替换现有的 `_buildInfoRow` 调用，确认三条信息等宽排列，各项目视觉一致。改变窗口/屏幕宽度后三项仍等分。

**2. Column 溢出问题解决**

在图书馆 App 的图书添加表单中，当表单项过多时可能导致 `Column` 内容超出屏幕高度，出现溢出警告（黑黄条纹）。请用三种方案解决这个问题：

- 方案 A：将外层 `Column` 替换为 `ListView`，表单项作为其 `children`
- 方案 B：用 `SingleChildScrollView` 包裹 `Column`
- 方案 C：在 `Scaffold` 的 `body` 中使用 `SingleChildScrollView` + `Column`，并在 `Scaffold` 上设置 `resizeToAvoidBottomInset: true`
- 对每种方案，写出一句话说明其适用场景和注意事项（如 `Expanded` 能否在方案 B 中使用）
- 验证：在 `BookFormScreen` 中模拟 10 个以上表单项（可重复现有字段），使用方案 B 确保所有表单项可滚动且不会溢出。特别测试键盘弹出时底部输入框是否被遮挡。

**3. IntrinsicHeight 的实际应用**

在图书馆 App 的首页中，需要将 `BookCard`（高度不定，取决于书名长度）和一段描述文字并排放在一个 `Row` 中，且要求两者高度一致（以较高者为准）。请实现：

- 使用 `IntrinsicHeight` 包裹 `Row`，使 `Row` 的所有子项高度统一
- 左侧：一个简化的 `BookCard`（只含封面图 + 书名，`CrossAxisAlignment.stretch` 使封面填满高度）
- 右侧：`Expanded` 包裹的描述文字 `Column`（书名 + 作者 + 简介摘要）
- 两者之间用 `VerticalDivider` 分隔
- 验证：新建 `lib/widgets/book_list_tile.dart`，用此组件替换 `BookCard` 在某个列表页中使用，准备不同书名长度的测试数据（3 字书名 vs 20 字书名），确认两行高度始终一致，`VerticalDivider` 贯穿整行。

---

> **下一步**: [Chapter 10 — 滚动与列表](./Chapter-10-滚动与列表.md)
