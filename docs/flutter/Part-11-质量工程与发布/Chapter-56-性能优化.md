...
> **上一章**: [Chapter 55 — 集成测试](./Chapter-55-集成测试.md)
> **下一章**: [Chapter 57 — DevTools 详解](./Chapter-57-DevTools详解.md)
...
**练习 1：排查 Library App 中缺少 const 构造的 Widget**
- 运行 `flutter analyze` 并启用 `prefer_const_constructors` lint 规则
- 将 Library App 中所有可标记为 `const` 的 Widget 构造函数添加 `const` 关键字
- 对比修改前后列表滚动帧率的变化
- 验证标准：`flutter analyze` 零警告，列表滑动帧率稳定在 60fps

**练习 2：使用 RepaintBoundary 优化 Library App 列表**
- 在 Library App 的图书列表页面中，为每张 `BookCard` 包裹 `RepaintBoundary`
- 使用 `debugRepaintRainbowEnabled = true` 观察启用前后的重绘范围差异
- 对比优化前后列表快速滚动时的帧率变化
- 验证标准：重绘范围仅限单张卡片而非整个列表，帧率明显改善

**练习 3：Library App 列表性能优化实战**
- 为长列表添加 `itemExtent`（固定高度项）或使用 `ListView.builder`
- 检查图片加载是否有合理缓存策略（如使用 `cached_network_image`）
- 使用 DevTools 的 "Enhance Tracing" 测量首帧时间和帧率
- 验证标准：首帧时间 < 200ms，列表滚动帧率保持在 58fps 以上，内存占用 < 200MB
